...
* Model names and field names should use `snake_case` format.
  * We require `snake_case` model names and field names. This is a recommended default in Postgres,
  and we agree that it's good form, so we're enforcing this format for all drivers for the time being.
* Join models should use a *composite primary key*.
  * For join models to be used transparently for relationships your join model must have
  a *composite primary key* that encompasses both foreign model foreign keys. For example, on a
  join model named `user_videos` you should have: `primary key(user_id, video_id)`, with both `user_id`
...
### Pro Tips
* Foreign key field names should end with `_id`.
  * Foreign key field names in the format `x_id` will generate clearer method names.
...
Flags:
  ...
  -b, --blacklist stringSlice   Do not include these models in your generated package
  ...
  -w, --whitelist stringSlice   Only include these models in your generated package
```
...
- Forgetting to exclude models you do not want included in your generation, like migration models.
...
In the event that you would like to build a query and specify the model yourself, you
can do so using `models.NewQuery()`:

```go
// Select all rows from the pilots model by using the From query mod.
...
From("pilots as p") // Specify the FROM model manually, can be useful for doing complex queries.
...
type CoolObject struct {
  // Don't specify a name, Bind will TitleCase the field
...
// Select specific fields of many jets
...
```go
// Retrieve pilot with all fields filled
pilot, err := models.FindPilot(db, 1)

// Retrieve a subset of field values
...
- Insert all fields **without** a database default value.
- Insert all fields with a non-zero value that have a database default value.

On the other hand, if a whitelist is provided, we will only insert the fields specified in the whitelist.

Also note that your object will automatically be updated with any missing default values from the
database after the `Insert` is finished executing. This includes auto-incrementing field values.
...
whitelist is to specify which fields in your object should be updated in the database.

If no `whitelist` argument is provided, `Update` will update every field except for
`primary key` fields.

If a `whitelist` argument is provided, `update` will only update the fields specified.
...
such as auto-incrementing field values.
...
The `conflictFields` argument allows you to specify the `ON CONFLICT` fields for Postgres.
For MySQL, this param will not be generated.

Note: Passing a different set of field values to the update component is not currently supported.
...
If your MySQL or Postgres models use enums we will generate constants that hold their values
...
An enum type defined like the above, being used by a model, will generate the following enums:
...
For MySQL we use `model name + field name + title cased value` to generate the const variable name.
...
to get the tests to pass in this event is to either use a parsable enum value or use a regular field
instead of an enum.

### Constants

The models package will also contain some structs that contain all of the model and field
names harvested from the database at generation time.

For model names they're generated under `models.ModelNames`:
...
// Usage example:
fmt.Println(models.ModelNames.Messages)
```
...
// Usage example:
fmt.Println(models.MessageFields.ID)
```
